# bea-rest-api Specification

## Purpose
Expose beA (besonderes elektronisches Anwaltspostfach) capabilities over REST that non-desktop
clients need to reach parity with the desktop client — notably exporting a whole message as a `.bea`
file for storage in a case, and computing case suggestions for an opened message from its reference
numbers, subject, body and sender. Read/triage, send and eEB endpoints are covered by the wider
`/v8/bea/**` surface.
## Requirements
