...
file number, an archived marker and the link description; the other case's short name
(Kurzrubrum) and the full description SHALL be reachable without opening the case, at least
as a tooltip.
...
- **THEN** the case header shows two entries below the subject field, each with the other
  case's file number and the link description, side by side
