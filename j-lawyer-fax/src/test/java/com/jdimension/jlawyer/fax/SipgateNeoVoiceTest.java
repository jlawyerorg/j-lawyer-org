...
public class SipgateNeoVoiceTest {
    ...
    @Test
    public void buildNeoVoiceUris_setsOwningChannelPerNumber() throws Exception {
        List<SipUri> uris = SipgateAPI.buildNeoVoiceUris(CHANNELS, PHONE_NUMBERS, DEVICES);

        SipUri n491 = uris.stream().filter(u -> "e4".equals(u.getUri()) && "+491".equals(u.getOutgoingNumber())).findFirst().get();
        SipUri n492 = uris.stream().filter(u -> "e4".equals(u.getUri()) && "+492".equals(u.getOutgoingNumber())).findFirst().get();
        assertEquals("number routed to c1 must carry channel c1", "c1", n491.getChannelId());
        assertEquals("number routed to c2 must carry channel c2", "c2", n492.getChannelId());
    }
    ...
    public void buildNeoCallBody_stripsLeadingPlusFromTargetAndCallerId() throws Exception {
        String body = SipgateAPI.buildNeoCallBody("e4", "+4915799912345", "+4928559388690", null);
    ...
    public void buildNeoCallBody_omitsCallerIdWhenEmpty() throws Exception {
        String body = SipgateAPI.buildNeoCallBody("e4", "+4915799912345", "", null);
    ...
    @Test
    public void buildNeoCallBody_includesChannelIdWhenProvided() throws Exception {
        String body = SipgateAPI.buildNeoCallBody("e4", "+4915799912345", "+4928559388690", "c1");

        JsonObject json = (JsonObject) Jsoner.deserialize(body);
        assertEquals("channelId must be sent verbatim (no plus stripping)", "c1", json.getString("channelId"));
    }

    @Test
    public void buildNeoCallBody_omitsChannelIdWhenEmpty() throws Exception {
        String body = SipgateAPI.buildNeoCallBody("e4", "+4915799912345", "+4928559388690", "");

        JsonObject json = (JsonObject) Jsoner.deserialize(body);
        assertFalse("channelId must be omitted when empty", json.containsKey("channelId"));
    }
}
